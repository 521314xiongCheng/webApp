...
export default {
  ...
  methods: {
    ...
    toActionPlan() {
      ...
      const params = {
        opaque: false,
        targetContactId: this.info.contactId,
      };
      this.$go(url, params);
    },
    async goAddNote() {
      const { data } = await this.$request.get(
        `${this.$ctx.baseApi}/notes/check?followContactId=${this.info.contactId}&contactId=${this.targetContactId}`
      );
      ...
      let params = {
        ...
        targetContactId: this.info.contactId,
        name: this.info.name,
        levelCode: this.info.careerLevelCode,
    ...
    async showActions() {
      ...
      if (this.info.setTargetIncomeAmt) {
        copyActions[4].name = "邀请修改收入目标";
      } else {
        copyActions[4].name = "邀请设置收入目标";
      }
      ...
      if (
        ...
        copyActions.forEach((item, index, arr) => {
          if (item.prop === "follow" || item.prop === "setTarget" || item.prop === "nbcAction") {
            arr.splice(index, 1);
          }
        });
