...
export default {
  ...
  methods: {
    ...
    async getFilterConditions() {
      ...
      let baseApi = this.$ctx.baseApi;
      const { data } = await this.$request.get(
        `${baseApi}/reports/employee/area/filter-conditions`
      );
      const {
        activityStatuses,
        careerLevels,
      ...
      const { cities, nsdProfiles, provinces } = areaWorkDTO;
      if (areaWorkDTO) {
        if (nsdProfiles) {
      ...
      this.filterConfig.forEach((o) => {
        ...
        if (o.prop === "provinces" && provinces) {
          o.options = provinces.map((l) => ({
            name: `${l}`,
            value: l,
          }));
        }
        if (o.prop === "cities" && cities) {
          o.options = cities.map((l) => ({
            name: `${l}`,
            value: l,
          }));
        }
