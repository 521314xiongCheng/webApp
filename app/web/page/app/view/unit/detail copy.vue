...
<template>
  ...
  >
    ...
    >
      ...
      <div class="unit-content-income" v-show="!isFullScreen">
        <div class="retail-wrap">
          ...
          <div class="retail retail-unit retail-flex-grow">
            <div class="retail-item">
            ...
            <div class="retail-item" @click="toEstimate">
    ...
    <!-- 数据更新提示时间 -->
    <van-popup v-model="popupTime" position="center" round closeable>
      <div class="popupTime">
        <p>规则</p>
    ...
    ></productionSheet>
    <van-popup v-model="popup" position="center" class="popupmodel">
      <div class="popbox">
        <!-- <img src="../../images/self.png" alt="" class="popimg"> -->
        <p class="popbox-title">提示：</p>
        <p class="popbox-mes">{{ describe }}</p>
        <div class="popbox-btn" @click="popup = !popup">确定</div>
      </div>
    </van-popup>
    <img
      v-if="isFullScreen"
...
export default {
    ...
    window.addEventListener("scroll", (e) => {
      ...
      if (!this.isFullScreen) {
        if (scrollTop >= unitContentIncome.clientHeight) {
          ...
          filterContent.style.cssText = `position: fixed;top: ${navBar.clientHeight}px;right: 0;z-index: 4;width: 100%;`;
  ...
  methods: {
    ...
    handleClickAvatar(row) {
      ...
      let copyActions = deepCopy(this.baseActions);
      console.log("copyActions2", copyActions);
      ...
      if (row.setTargetIncomeAmt) {
        copyActions.forEach((item, index, arr) => {
          if (item.prop === "setTarget") {
            arr.splice(index, 1);
          }
        });
      }
      if (row.contactId.toString() === this.targetContactId) {
        copyActions.forEach((item, index, arr) => {
          if (
            item.prop === "follow" ||
            item.prop === "setTarget" ||
      ...
      if (this.accountType === "operation") {
        ...
        if (Number(row.careerLevelCode) < 60) {
          copyActions.forEach((item, index, arr) => {
            if (item.prop === "call") {
              arr.splice(index, 1);
            }
      ...
      // // 关闭35以下人的目标设置功能
      // if (this.levelCode <= 35 || row.careerLevelCode <= 35) {
      //   copyActions.forEach((item, index, arr) => {
      //     if (item.prop === "setTarget") {
      //       arr.splice(index, 1);
      //     }
      ...
      if (!n) {
        ...
        this.$nextTick(() => {
          ...
          if (
            ...
            filterContent.style.cssText = `position: fixed;top: ${this.navBarClientHeight}px;right: 0;z-index: 4;width: 100%;`;
            tableHeaderWrap.style.cssText = `position: fixed;top: ${
