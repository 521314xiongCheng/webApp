  ...
  methods: {
    ...
    async getNoteInfo() {
      ...
      const { data } = await this.$request.get(
        `${baseApi}/notes/${this.$route.query.noteId}`
      );
       if (!data) {
        Dialog.alert({
      ...
      if (this.note.noteContent) {
        if (this.$route.query.noteId) {
          ...
          const res = await this.$request.put(
            `${baseApi}/notes/${noteId}?updateType=1&followContactId=${this.$route.query.targetContactId}&noteDate=${this.note.noteDate}&noteContent=${this.note.noteContent}&needPush=${this.note.needPush}`
          );
          if(res){
          ...
          const res = await this.$request.post(
            `${baseApi}/notes?followContactId=${this.$route.query.targetContactId}&noteDate=${this.note.noteDate}&noteContent=${this.note.noteContent}&needPush=${this.note.needPush}`,
            params
          );
          if(res){
            this.loadChange=true
