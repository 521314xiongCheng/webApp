        ...
        >
          ...
          >
            <template slot="header">
              <div><span>资深</span><br /><span>经销商</span></div>
            </template>
            <template slot-scope="{ row }">
              <div v-if="row.directorContactId" class="user" @click="handleClickName(row)">
                <div class="user-info">
                  <div class="user_name">
                    <span class="climp">{{ row.directorName }}</span>
                ...
                <div
                  class="tags"
                  v-if="targetContactId === row.directorContactId.toString()"
                >
                  <span class="level">{{ row.directorCareerLevelCode }}</span>
                  <span class="status">{{ row.directorActivityStatusCode }}</span>
                  <span class="self">本人</span>
                </div>
                <div class="no-self" v-else>
                  <span class="level">{{ row.directorCareerLevelCode }}</span>
...
<script>
import { Search, List, Button, Icon, Dialog, ActionSheet } from "vant";
...
export default {
  ...
  methods: {
    ...
    handleClickName(row){
      this.showDirectorActions = true;
      this.currentData = row;
      this.sheetType = 'director';
    },
    async share() {
    ...
    async onSelect(item) {
      ...
      switch (item.prop) {
        ...
        case "profile": //个人详情
          ...
          const params = {
            opaque: false,
            targetContactId: this.sheetType == 'director' ? this.currentData.directorContactId : this.currentData.contactId,
            levelCode: this.sheetType == 'director' ? this.currentData.directorCareerLevelCode : this.currentData.careerLevelCode,
              //跟进人
        ...
        case "assess": //查看目标评估
          ...
          break;
        case "call": //拨打电话
          window.location.href = this.sheetType == 'director' ? `tel:${this.currentData.directorMobile}`  : `tel:${this.currentData.mobile}`;
          break;
      }
      this.showDirectorActions = false
